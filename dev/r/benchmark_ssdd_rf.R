## benchmark_ssdd_rf.R
##
## Benchmark 2: Three-metric SSDD RF. Uses terrain plus KD_raw, BA_raw,
## and DP_raw — the three SSDD metrics that are candidate replacements for
## Kenny's build_dens (KD ≈ kernel density), area_ext_build (BA ≈ building
## area), and distance_to_nearest_building (DP ≈ distance proxy). No
## blending; metrics fed directly into RF.
##
## Purpose: establish whether the SSDD metrics collectively carry at least
## as much signal as build_dens alone (benchmark 1), using the same RF
## approach and spatial-CV setup.
##
## Features: KD_raw, BA_raw, DP_raw, elevation, slope, aspect
##
## Inputs:
...
SSDD_COLS    <- c("KD_raw", "BA_raw", "DP_raw")
...
out_path <- "dev/r/benchmark_ssdd_rf_results.csv"
write_csv(results, out_path)
message(sprintf("Wrote %s", out_path))

pred_path <- "dev/r/benchmark_ssdd_rf_predictions.csv"
